...
func draw_arrow_for_frames(pos: Vector3, normal: Vector3, color := Color.WHITE, thickness = 1.0, frame_duration: int = -1, parent: Node = null) -> DebugArrow:
	return draw_arrow_between_for_frames(pos, pos + normal, color, thickness, frame_duration, parent)
...
func draw_arrow_between_for_frames(from: Vector3, to: Vector3, color := Color.WHITE, thickness = 1.0, frame_duration: int = -1, parent: Node = null) -> DebugArrow:
	var debug_arrow := debug_arrow_scene.instantiate() as DebugArrow
	debug_arrow.tail_position = from
	debug_arrow.head_position = to
	debug_arrow.color = color
	debug_arrow.thickness = thickness
	debug_arrow.temporary = frame_duration >= 0
	debug_arrow.frame_duration = frame_duration
	if parent:
		parent.add_child(debug_arrow)
	else:
		add_child(debug_arrow)
